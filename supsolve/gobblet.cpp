...
public:
    ...
    int action_from_move(Move m) {
	    int _ACTION_OFFSET = 3 * 9;
	    if (m.start < 0) {
		    return (-m.start-1)*9 + m.end;
	    }
	    return _ACTION_OFFSET + m.start*9 + m.end;
    }

    void encode_board(int *board, State s) {
    	for (int i = 0; i < 54; i++) {
		board[i] = 0;
	}
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			...
			unsigned owner = 0;
				...
				if (owner != 0) {
					int idx = (row*3+col)*6 + size*2 + (owner-1);
					board[idx] = 1;
				}
			}
		}
    ...
    void dump_best() {
	std::fstream fout;
	fout.open("solve.csv", std::ios::out);

	int board[54];
        for (State& s : hash_map) {
		if (s == STATE_EMPTY) {
			continue;
		}
		if (get_terminal_value(s) != 0) {
			continue;
		}

		Move best = best_move(s);
		int action = action_from_move(best);
		encode_board(board, s);

		for (int i = 0; i < 54; i++) {
			fout << board[i] << ",";
		}
		fout << std::setfill('0') << std::setw(3) << action << std::endl;
	}
    }
